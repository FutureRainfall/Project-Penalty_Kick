...
## 说明
游戏使用*python*编写。所有所需文件已放在pack文件夹中。主执行模块`main.py`，加密模块`data_encrypt.py`，登录模块`Login.py`。`users.pk`存储了加密的用户密码，`save.pk`则加密存储了所有用户的游戏存档信息。

**注意：** 使用时将所有文件置于**相同路径**下，运行`main.py`即可开始游戏！
